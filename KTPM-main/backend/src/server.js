...
io.on('connection', (socket) => {
  ...
  socket.on('stream:join', (data) => {
    ...
    if (global.io) {
      io.to(`stream:${streamId}`).emit('stream:viewerJoined', {
        streamId,
        viewerCount: getStreamRoomSize(`stream:${streamId}`)
      });
    }
  });
  ...
  // Viewer sends a chat message in stream
  socket.on('stream:chatMessage', (data) => {
  ...
  // Broadcaster sends a live chat message
  socket.on('stream:broadcasterMessage', (data) => {
    const { streamId, userId, userName, message } = data;
    if (!streamId) return;
  ...
  // Helper function to get room size
  function getStreamRoomSize(roomName) {
