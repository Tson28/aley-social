...
export class AleyAskComponent implements OnInit, OnDestroy {
    ...
    this.llamaService.sendMessage(messageText)
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.errorMessage = error.error || 'Có lỗi xảy ra khi xử lý yêu cầu.';
          setTimeout(() => this.scrollToBottom(), 100);
        }
      });
  }
  ...
  formatMessage(text: string): SafeHtml {
    ...
    // Headers: # Header
    formatted = formatted.replace(/^### (.+)$/gm, '<h4 class="ai-heading">$1</h4>');
    formatted = formatted.replace(/^## (.+)$/gm, '<h3 class="ai-heading">$1</h3>');
    formatted = formatted.replace(/^# (.+)$/gm, '<h2 class="ai-heading">$1</h2>');

    // Numbered lists: 1. item
    formatted = formatted.replace(/^(\d+)\. (.+)$/gm, '<li class="numbered-item">$2</li>');
    formatted = formatted.replace(/(<li class="numbered-item">.*<\/li>\n?)+/g, '<ol class="ai-list numbered">$&</ol>');

    // Bullet lists: - item or * item
    formatted = formatted.replace(/^[-*] (.+)$/gm, '<li class="bullet-item">$1</li>');
    formatted = formatted.replace(/(<li class="bullet-item">.*<\/li>\n?)+/g, '<ul class="ai-list bullet">$&</ul>');

    // Convert newlines to <br> (but preserve intentional double newlines)
